...
import { ChevronRight, MapPin, Trash2, Download, AlertCircle, CheckCircle2, Circle, ExternalLink, Building2, Plus, ArrowLeft, DollarSign, Ruler, Home, Wrench, FileCheck, BookOpen, Edit3, RotateCcw, Copy, Edit } from 'lucide-react';
...
const blankProject = () => ({
  ...
  parentLotSize: '',
  subdivisionStatus: 'titled', // 'proposed' | 'approved' | 'titled'
  lots: [blankLot('QLD')],
  completedSteps: {}, costOverrides: {}, createdAt: null,
});
...
export default function SDAPortal() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const result = await window.storage.get('sda-projects-v4');
  ...
  const saveProjects = async (updated) => {
    setProjects(updated);
    try {
      await window.storage.set('sda-projects-v4', JSON.stringify(updated));
  ...
  return (
    <div style={{ maxWidth: 1280, margin: '0 auto', padding: '60px 32px' }}>
      <div style={{ marginBottom: 56, maxWidth: 720 }}>
        <div style={{ fontSize: 10, letterSpacing: 2.5, color: '#666', marginBottom: 16 }} className="sans">FOR DEVELOPERS, BUILDERS & PLANNERS · v4</div>
